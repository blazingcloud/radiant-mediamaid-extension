...
class Mediamaid < ActiveRecord::Base
  LOCAL_STORAGE_DIRECTORY = "#{RAILS_ROOT}/public"
  ...
  def size_format
    ...
    if self.mediamaid_file_size > 1000000
      size           = mediamaid_file_size/1000000
      display_format = 'KB'
    elsif self.mediamaid_file_size > 1000
      size           = mediamaid_file_size/100
      display_format = 'MB'
    else
      size = mediamaid_file_size
    end
    size.to_s + display_format
  end

  def file_exists?
    file_path = LOCAL_STORAGE_DIRECTORY +  self.mediamaid.url(:thumb).to_s
    File.exists?(file_path.to_s)
  end
